...
# PROJECT ASSISTANT
# =========================================================
# The assistant is LLM-driven. No keyword-based routing is used for normal
# questions. The LLM receives structured project/ML context and decides what
# information is relevant to the user's question.
# =========================================================

def _assistant_word_count_request(message):
    import re
    q = " ".join(str(message).lower().split())
    patterns = [
...
def _assistant_local_fallback(analysis, project, message):
    """Data-grounded resilience path only; the primary path remains the LLM."""
    project = project if isinstance(project, dict) else {}
    analysis = analysis if isinstance(analysis, dict) else {}
    p = analysis.get("project") if isinstance(analysis.get("project"), dict) else project

    name = p.get("project_name") or p.get("name") or "Project"
    pid = p.get("project_id") or p.get("id") or "N/A"
    progress = p.get("physical_progress", p.get("physical_progress_percent"))
    cost = analysis.get("cost_prediction") or {}
    time_data = analysis.get("time_prediction") or {}
    risk = analysis.get("risk") or {}

    # Build a factual sentence from whatever fields are actually available.
    parts = [f"Project {pid} is {name}"]
    if progress not in (None, ""):
        try:
            parts.append(f"with {float(progress):.1f}% recorded physical progress")
        except (TypeError, ValueError):
            parts.append(f"with {progress} recorded physical progress")
    if risk.get("risk_level") not in (None, ""):
        parts.append(f"and model risk level {risk['risk_level']}")
    if cost.get("predicted_final_cost") is not None:
        parts.append(f"with predicted final cost {_fmt_currency(cost['predicted_final_cost'])}")
    if time_data.get("predicted_delay_days") is not None:
        parts.append(f"and predicted delay of {_fmt_number(time_data['predicted_delay_days'], 1)} days")

    text = ". ".join(parts) + "."
    requested = _assistant_word_count_request(message)
    if requested is not None:
        words = text.split()
        if len(words) > requested:
            return " ".join(words[:requested]).rstrip(".,;:") + "."
        # Add unique factual-context words only; never repeat a word.
        extras = ["The", "available", "records", "provide", "the", "current", "project", "context", "for", "assessment"]
...
def _build_analysis_from_project_id(project_id):
    """
    Build complete project context for the AI assistant.

    Fast path: reuse a recent ML prediction already generated by
    ``predict_project_api``.

    Cache-miss path: run the EXISTING prediction pipeline once, cache its
    result, and give that real ML analysis to the LLM. This makes questions
    about risk, cost, delay, recommendations, uncertainty, and historical
    evidence answerable without adding keyword-based intent logic.

    If ML inference fails, keep the assistant available with the real project
    record so factual project questions can still be answered. The failure is
    logged and the missing ML analysis is not fabricated.
    """
    ...
    # Cache miss: generate the same ML analysis used by the prediction API.
    # This is deliberately the existing prediction pipeline, not a new
    # assistant-specific or keyword-driven risk engine.
    try:
        generated = predict_project(project)

        # Keep the additive cost-escalation analysis consistent with the
        # existing prediction endpoint before caching the result.
        if isinstance(generated, dict):
            cost_result = generated.get("cost_prediction")
            if isinstance(cost_result, dict) and "cost_escalation_analysis" not in cost_result:
                historical_projects = cost_result.get("historical_projects") or []
                historical_overruns = [
                    item.get("cost_overrun_percent")
                ...
                generated["cost_prediction"] = cost_result

            _cache_assistant_analysis(project_id, generated)
            return project, generated

    except Exception as exc:
        import traceback
        print(f"[PROJECT_ASSISTANT] ML context generation failed for project {project_id}: {exc}")
        traceback.print_exc()

    # Resilience path: only factual project data, never invented ML values.
    return project, {
        "project": project,
        "source": "current_project_record_only_ml_unavailable",
    }
